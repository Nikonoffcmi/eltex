...
void Display_contacts(Manual *manual) {
    ...
    for(int i = 0; i < manual->users; i++) {
        printf("\n%d. %s %s (ID: %d)", 
                i+1, 
                manual->list[i].person.name, 
                manual->list[i].person.last_name, 
                manual->list[i].ID);
    }
    printf("\n");
}
...
void Add_contact_menu(Manual *manual) {
    ...
    Contact *new = &manual->list[manual->users-1];
    ...
    while(1) {
        ...
        switch(choice) {
            case 1: {
                ...
                Update_person(target, name, last_name, 
                            *patronymic ? patronymic : NULL,
